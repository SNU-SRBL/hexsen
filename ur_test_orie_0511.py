...
def main():
    ...
    base_position = (-0.63800, 0.11738, 0.04562)  # in meters
    base_orientation = (-180, 0, -180)  # in degrees
    
    # Generate 20 points in upper hemisphere (convert mm to m)
    MM_TO_M = 0.001

    floor1 = [(x*MM_TO_M, -50*MM_TO_M, 0) for x in [200, 100, -100, -200]] + [(x*MM_TO_M, -150*MM_TO_M, 0) for x in [-100, 0, 100]]
    floor2 = [(x*MM_TO_M, -50*MM_TO_M, 75*MM_TO_M) for x in np.linspace(200, -200, 5)]\
      + [(x*MM_TO_M, -150*MM_TO_M, 75*MM_TO_M) for x in np.linspace(-100, 100, 3)]
    floor34 = [(x*MM_TO_M, -50*MM_TO_M, 150*MM_TO_M) for x in np.linspace(100, -100, 3)]\
      + [(0, -150*MM_TO_M, 150*MM_TO_M), (0, -50*MM_TO_M, 225*MM_TO_M)]
    sphere_points = floor34
