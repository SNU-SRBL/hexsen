...
async def run_ble():
    ...
    print(f"Scanning for {DEVICE_NAME} ({TARGET})...")
    ...
    # Try to find device by address first, then by name
    for d in devices:
        print(f"  Found: {d.name} ({d.address})")
        if d.address.upper() == TARGET.upper():
            device = d
            print(f"✓ Matched target device: {d.name}")
            break
    ...
    if not device:
        for d in devices:
            if d.name and DEVICE_NAME in d.name:
                device = d
                print(f"✓ Matched by name: {d.name}")
                break

    if not device:
        print(f"Could not find {DEVICE_NAME} or {TARGET}")
        return
