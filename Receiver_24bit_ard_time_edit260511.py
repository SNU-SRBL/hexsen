    ...
    if csv_writer:
        for i in range(8):
            ...
            if current_time - last_print_time >= PRINT_INTERVAL:
                print(f"[{formatted_time}s] Arduino µs: {ard_micros} | CH1: {voltages[0]:.4f}mV | CH2: {voltages[1]:.4f}mV | CH3: {voltages[2]:.4f}mV | CH4: {voltages[3]:.4f}mV | CH5: {voltages[4]:.4f}mV | CH6: {voltages[5]:.4f}mV") # V -> mV conversion
...
async def run_ble():
    ...
    async with BleakClient(device) as client:
        ...
        csv_writer.writerow(['Relative_Time(s)', 'Ard_Micros', 'CH1(mV)', 'CH2(mV)', 'CH3(mV)', 'CH4(mV)', 'CH5(mV)', 'CH6(mV)']) # V -> mV conversion
